...
impl Contract {
    ...
    pub fn get_vendor_metadata(&self, vendor_id: AccountId) -> VendorMetadata {
        self.account_details_by_id.get(&vendor_id).expect("No vendor found").vendor_data.expect("No vendor data found").metadata
    }
    ...
    pub fn get_items_for_vendor(&self, vendor_id: AccountId, from_index: Option<U128>, limit: Option<u64>) -> Vec<InternalVendorItem> {
        let vendor_data = self.account_details_by_id.get(&vendor_id).expect("No vendor found").vendor_data.expect("No vendor data found");
